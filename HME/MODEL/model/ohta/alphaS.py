...
def build_alphaS(
    ...
    **_,
    ...
    kernel_size = 5
    out_kernel_size = 3
    num_channels = int(64 * size_rate)
    cnet_out_dim = int(160 * size_rate)
    encoder_dim = int(256 * size_rate)
...
class AlphaS(nn.Module):
    def __init__(
        ...
        self.out_conv = OutConvS(encoder_dim, num_channels, out_kernel_size)
    ...
    def __init__(
        ...
        self.blocks.append(
            nn.Sequential(
                ...
                ConvLayer(num_channels, num_channels * 2, kernel_size, 2),
            )
        )
        self.blocks.append(
            nn.Sequential(
                ConvLayer(num_channels * 2, num_channels * 2, kernel_size, 1),
                ConvLayer(num_channels * 2, num_channels // 2, kernel_size, 2),
            )
        )
        self.blocks.append(
            nn.Sequential(
                ConvLayer(num_channels // 2, num_channels // 16, kernel_size, 1),
